...
class WeatherETR:
    def __init__(self, 
        ...
        """
        Initializes the WeatherETR class with default elevation, latitude, 
    ...
    def preprocess_data(self):
        """
        Preprocesses the DataFrame by handling missing values and calculating necessary 
        parameters.
        """
        if self.df is None:
            raise ValueError(
                "DataFrame is empty. Please load data first using load_data()."
                )
        ...
        required_columns = ['TIMESTAMP', 'Tmax', 'Tmin', 'SolarRad', 'VaporPressure', 
                            'WindSpeed', 'DOY']
        ...
        self.df[required_columns[1:]] = self.df[required_columns[1:]].interpolate(
    ...
    def calculate_daily_etr(self):
        """
        Calculates daily reference evapotranspiration (ETr) and adds it to the 
        DataFrame.
        """
        if self.df is None:
            raise ValueError(
                "DataFrame is empty. Please load and preprocess data first."
                )
    ...
    def calculate_kc(self, eta_columns=None, etr_column='ETr'):
        """
        Calculates the crop coefficient (Kc) using the formula Kc = ETa / ETr for each 
        specified ETa column.
        
        Args:
            eta_columns (list): List of column names for Actual Evapotranspiration 
                                (ETa).
                                If None, it will automatically detect columns ending 
                                with '_ETa'.
        ...
        if etr_column not in self.df.columns:
            raise ValueError(f"'{etr_column}' column is missing. Please ensure ETr is "
                             "calculated before calculating Kc.")
        ...
        if eta_columns is None:
            ...
            eta_columns = [col for col in self.df.columns if col.endswith('_ETa') 
                           and not col.endswith('_Cumulative_ETa')]
    ...
    def plot_etr_vs_eta(self, eta_columns=None):
        """
        Plots a time series of ETa and ETr values, including points on the lines, saves 
        it as an HTML file, and returns the Plotly figure.
        
        Args:
            eta_columns (list): List of column names for Actual Evapotranspiration 
                                (ETa).
                                If None, it will automatically detect columns ending 
                                with '_ETa' but not '_Cumulative_ETa'.
        ...
        # Automatically detect columns ending with '_ETa' but exclude '_Cumulative_ETa'
            eta_columns = [col for col in self.df.columns if col.endswith('_ETa') 
                           and '_Cumulative_ETa' not in col]
        
        fig = go.Figure()
        
        # Plot ETr with points and lines
        fig.add_trace(go.Scatter(
            x=self.df['TIMESTAMP'], y=self.df['ETr'], mode='lines+markers', name='ETr'
        ))
        ...
        for eta_col in eta_columns:
            if eta_col in self.df.columns:
                fig.add_trace(go.Scatter(
                    x=self.df['TIMESTAMP'], y=self.df[eta_col], mode='lines+markers', 
                    name=eta_col
                ))
        ...
        output_filename_html = os.path.join(self.output_directory, 
                                            f"ETa_vs_ETr_{timestamp}.html")
    ...
    def plot_kc_with_fit(self, kc_columns=None):
        """
        Plots a time series of Kc values, saves it as an HTML file, and returns the 
        Plotly figure.
        ...
        # Filter the DataFrame for the period between planting and harvest, if defined
        if self.planting_date and self.harvest_date:
            df_filtered = self.df[(self.df['TIMESTAMP'] >= self.planting_date) & 
                                  (self.df['TIMESTAMP'] <= self.harvest_date)]
        else:
            df_filtered = self.df
        ...
        for kc_col in kc_columns:
            if kc_col in df_filtered.columns:
                fig.add_trace(go.Scatter(
                    x=df_filtered['TIMESTAMP'], y=df_filtered[kc_col], mode='markers', 
                    name=kc_col
                ))
                ...
                fig.add_trace(go.Scatter(
                    x=df_filtered['TIMESTAMP'].iloc[:len(poly_vals)], y=poly_vals, 
                    mode='lines', name=f'{kc_col} Fit'
                ))
